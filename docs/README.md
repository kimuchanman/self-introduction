...
プロダクトを作っていると、知らなかった世界の一端を知れます。1社目では不動産や宅建、市場のことを、その後は介護のドメインを、何も知らないところからプロダクト作りを通して学んできました。そうして得た知識が設計や日々の判断に使える知恵に変わっていく感覚があって、これはなかなか他では得がたい面白さだと思っています。

また、プロダクトには忖度のないフィードバックが返ってきます。市場に出した機能への反応もそうですし、プラットフォームエンジニアリング的な仕事では、開発者向けに出した機能の使われ方がログとして正直に出てきます。ログは嘘をつかないので、そこにある事実と向き合うのは面白いです。

そしてプロダクトは共通言語になってくれます。プロダクトを話題にすると、チームの内外を問わずその人の人となりや思想背景まで知れますし、市場との間でも「こう使われるはず」と思想を込めて出したものが思ったように受け取られなかったりする。この往復自体が、プロダクトを介した開発とユーザーのコミュニケーションだと思っていて、この構図がとても好きです。

知らない世界を教えてくれて、正直な反応を返してくれて、人や市場との架け橋にもなってくれる。プロダクトは作っている側にもこれだけ多くのものを返してくれるので、本当に好きです。
```
...
カイポケリニューアルの開発推進チームを発足&所属し、チームトポロジー文脈で言うところの platform engineering に近い立ち位置で、Stream-aligned チームの「開発効率」と「開発者体験（DX）」を向上することを目指して活動しています。
  ...
  - CI/CD ワークフローの最適化とコスト削減
    ...
    - 依存パッケージのサプライチェーン検証を Renovate 実行時に集約し、通常の CI は lockfile 信頼設定で高速化
