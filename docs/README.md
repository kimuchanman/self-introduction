...
これまで AWS SES を使ってメール送信していたのを、AWS SESv2 を利用。SendBulkEmail を利用して、これまで 1 hour かかるような処理を 18 min 程度に短縮（約1/3の短縮）
それによりショップオーナーが1日に送ることができるメルマガの上限を安全に緩和することができた。
...
その後は CRM 関連機能を統括的に見つつメルマガ App やアンケート App など様々な BASE の Apps 開発もしていました。
