...
プロダクトを作っていると、知らなかった世界の一端を知れます。
不動産や宅建のこと、決済の仕組み、介護のドメインなど、どれも何も知らないところからプロダクト作りを通して学んできました。
本を読んで外から眺めるのとは違って、作っていると業務の実態や制約と否応なく向き合うことになります。
すると、人はどう動くのか、社会はどういう仕組みで回っているのか、これから人や業界や世界はどうなっていくのか、そういうものが少しずつ見えてきます。
プロダクトはこの世の実相に接続する一つのコネクタだと思っていて、この接続の感覚が好きです。
そうして得た知識が設計や日々の判断に使える知恵に変わっていく感覚もあって、これはなかなか他では得がたい面白さだと思っています。

同時に、プロダクトは自分たちが現実世界をどう捉えているかを映す鏡でもあります。
モジュールの切り方や互いの関係性には、作り手の認識がそのまま出ます。
たとえば事業所を一つの主体として切っていたら、介護保険上の指定事業所は法人・拠点・サービス種別ごとに別の存在で、自分たちが会社の支店のような感覚で捉えていたことが見えました。
思い込みが構造として外に出てくるので、設計を直すことで認識のほうも直せます。

また、プロダクトには忖度のないフィードバックが返ってきます。
市場に出した機能への反応もそうですし、プラットフォームエンジニアリング的な仕事では、開発者向けに出した機能の使われ方がログとして正直に出てきます。
ログは嘘をつかないので、そこにある事実と向き合うのは面白いです。

そしてプロダクトは共通言語になってくれます。
プロダクトを話題にすると、チームの内外を問わずその人の人となりや思想背景まで知れますし、市場との間でも「こう使われるはず」と思想を込めて出したものが思ったように受け取られなかったりします。
この往復自体が、プロダクトを介した開発とユーザーのコミュニケーションだと思っていて、この構図がとても好きです。

知らない世界を教えてくれて、正直な反応を返してくれて、人や市場との架け橋にもなってくれます。
プロダクトは作っている側にもこれだけ多くのものを返してくれるので、本当に好きです。
```
...
- GraphQL (Apollo / gql.tada) / REST API
...
カイポケリニューアルの開発推進チームを発足&所属し、チームトポロジー文脈で言うところの platform engineering に近い立ち位置で、Stream-aligned チームの「開発効率」と「開発者体験（DX）」を向上することを目指して活動しています。
  ...
  - CI/CD ワークフローの最適化とコスト削減
    ...
    - ビルド成果物のハッシュ比較による差分検証をソースマップの正規化を含めて全アプリに展開し、変更影響の事前検知を実現
    - 依存パッケージのサプライチェイン検証を Renovate 実行時に集約し、通常の CI は lockfile 信頼設定で高速化
    - Renovate CI 失敗時の自動チケット作成・PR クローズなど、依存更新の運用を自動化
    - Vite アプリ共通の Bundle Analyzer プラグインを開発し、全アプリの日次バンドル分析ワークフローを構築
  - モノレポ基盤の設計と最適化
  ...
  - コード品質と DX 向上
    ...
    - コーディングガイドラインを静的 lint ルールへ変換する仕組みを構築（oxlint カスタムルールを 14+ 件実装し、lint 化済みの規則 20+ 件をガイドライン文書から削除して単一情報源化）
    - レイヤー間の依存境界を検査する lint ルールを自社開発（動的 import・re-export・分割代入経由の参照も検知）
    - 環境変数を型安全に管理する共通パッケージを設計・導入（直接参照を禁止する lint ルールとセットで運用）
  ...
  - AI/自動化ツールの導入と開発の生産性向上
    ...
    - コードレビューの指摘からコーディングガイドライン候補を抽出し lint ルール化まで行う週次のエージェントワークフローを構築
    - flaky な VRT を検出して修正 PR の作成と Slack 通知まで行う自動修正エージェントを構築
    - コミットトレイラーによる AI・スキル利用状況の記録と集計基盤を構築し、ハーネス改善のフィードバックループを確立
    - Model Context Protocol（MCP）サーバーの開発・運用
  ...
  - 新規プロダクト立ち上げへの参画（2026/09〜）
    - ドメインモデル設計を ADR として実装に先行して整備（適用条件モデルや一括作成の設計など複数本を執筆し、レビューを経て実装と整合）
    - AI コーディングエージェント前提の開発基盤を立ち上げ初期から整備（ドメイン知識・トラブルシューティングの文書化、スキル群の移植）
    - バックエンドのドメインサービス切り出しや、共有環境を壊さない検証用データ投入スクリプトの整備
